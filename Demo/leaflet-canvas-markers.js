    ...
    const CanvasMarker = L.CircleMarker.extend({
        _updatePath() {
            ...
            if (!this.options.img.el) {
                ...
                img.onload = () => {
                    //this.redraw();//現状では強制redrawしなくても問題なし
                };
                img.onerror = () => {
        ...
        _containsPoint: function(p) {
          if(this.options.pointoffset){
            return L.CircleMarker.prototype._containsPoint.call(this, p.subtract([this.options.pointoffset.x, this.options.pointoffset.y]));
          }
        }
